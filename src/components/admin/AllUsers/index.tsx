...
const AllUsers = () => {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/admin/get-users/?page=${currentPage}${searchParam}`);
        ...
        <table className="min-w-full border-separate border-spacing-y-2">
          <thead>
            <tr className="text-md text-gray-600">
              <th className="pb-4 px-4 border-b text-start font-medium">User Name</th>
              <th className="pb-4 px-4 border-b text-start font-medium">Email Address</th>
              <th className="pb-4 px-4 border-b text-center font-medium">Added On</th>
              <th className="pb-4 px-4 border-b text-center font-medium">No Of Downloads</th>
              <th className="pb-4 px-4 border-b text-center font-medium">Action</th>
            </tr>
          </thead>
          <tbody>
            {users.map((user, index) => (
              <tr key={user.id} className="text-primary bg-[#F5F5F5]">
                ...
                <td className="py-3 px-4 text-start text-lg font-medium">{user.email}</td>
                ...
                <td className="py-3 px-4 text-center text-lg font-medium rounded-r-xl text-blue-500">
                  <span className="border-b-blue-500 border-b font-semibold">
                    View Details
                  </span>
                </td>
              </tr>
